...
# from Object_function_1.2 import objective_function, range_bound, planting_dispersion_decay, area_deviation, cv
from
...
def compute_reward(action):
    ...
    # return area_deviation(action)
    # return cv(action)

# 分批处理的目标函数计算（顺序执行）
def analyze_objective_distribution(action_space_shape, num_samples=50, batch_size=10):
    rewards = []
    ...
    for batch_num in tqdm(range(num_batches + (1 if remainder > 0 else 0))):
        ...
        # 顺序计算每个动作的目标函数
        batch_rewards = [compute_reward(action) for action in actions]
...
analyze_objective_distribution(action_space_shape=(82, 41, 7), num_samples=1000, batch_size=10)
