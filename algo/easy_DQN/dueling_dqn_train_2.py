...
class DuelingCropQNetwork(nn.Module):
    def __init__(self, input_shape, output_shape):
        ...
        # Flatten the input dimensions (e.g., input_dim * 4)
        self.input_dim = np.prod(input_shape)  # Flattened input dimension
        self.output_shape = output_shape  # Action space dimensions (e.g., input_dim * 7)

        # Shared layers (you can adjust the number of neurons in these layers)
        self.fc1 = nn.Linear(self.input_dim, 512)  # Reduced to 512 units

        # Value and Advantage streams
        self.fc_value = nn.Linear(512, 256)        # Value stream layer with 256 units
        self.fc_advantage = nn.Linear(512, 256)    # Advantage stream layer with 256 units

        self.value = nn.Linear(256, 1)             # Value output
        self.advantage = nn.Linear(256, np.prod(output_shape))  # Advantage output (680 * 7)

    def forward(self, x):
        # Flatten the input to a 2D tensor (batch_size, input_dim)
