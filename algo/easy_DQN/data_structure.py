...
class data():
    ...
    def matrix2vector(self, matrix):
        liangshi = matrix[0:25, 0:15].flatten()
        bunaihanshucai_shuijiaodi = matrix[25:33, 16:34].flatten()
        bunaihanshucai_dapeng1 = matrix[53:61, 16:34].flatten()
        bunaihanshucai_dapeng2 = matrix[77:81, 16:34].flatten()
        naihan_shucai = matrix[33:53, 34:37].flatten()
        shiyongjun = matrix[61:77, 37:41].flatten()

        vector = np.concatenate([liangshi, bunaihanshucai_shuijiaodi, bunaihanshucai_dapeng1,
                                 bunaihanshucai_dapeng2, naihan_shucai, shiyongjun])
        return vector
    ...
    def vector2matrix(self, vector):
        ...
        # 获取每个区域的大小
        liangshi_size = 25 * 15
        bunaihanshucai_shuijiaodi_size = 8 * 18
        bunaihanshucai_dapeng1_size = 8 * 18
        bunaihanshucai_dapeng2_size = 4 * 18
        naihan_shucai_size = 20 * 3
        shiyongjun_size = 16 * 4

        # 从向量中提取各个部分
        liangshi = vector[:liangshi_size].reshape(25, 15)
        bunaihanshucai_shuijiaodi = vector[liangshi_size:liangshi_size + bunaihanshucai_shuijiaodi_size].reshape(8, 18)
        bunaihanshucai_dapeng1 = vector[liangshi_size + bunaihanshucai_shuijiaodi_size:
                                        liangshi_size + bunaihanshucai_shuijiaodi_size + bunaihanshucai_dapeng1_size].reshape(
            8, 18)
        bunaihanshucai_dapeng2 = vector[liangshi_size + bunaihanshucai_shuijiaodi_size + bunaihanshucai_dapeng1_size:
                                        liangshi_size + bunaihanshucai_shuijiaodi_size + bunaihanshucai_dapeng1_size + bunaihanshucai_dapeng2_size].reshape(
            4, 18)
        naihan_shucai = vector[
                        liangshi_size + bunaihanshucai_shuijiaodi_size + bunaihanshucai_dapeng1_size + bunaihanshucai_dapeng2_size:
                        liangshi_size + bunaihanshucai_shuijiaodi_size + bunaihanshucai_dapeng1_size + bunaihanshucai_dapeng2_size + naihan_shucai_size].reshape(
            20, 3)
        shiyongjun = vector[-shiyongjun_size:].reshape(16, 4)

        # 还原到矩阵的相应位置
        matrix[0:25, 0:15] = liangshi
        matrix[25:33, 16:34] = bunaihanshucai_shuijiaodi
        matrix[53:61, 16:34] = bunaihanshucai_dapeng1
        matrix[77:81, 16:34] = bunaihanshucai_dapeng2
        matrix[33:53, 34:37] = naihan_shucai
        matrix[61:77, 37:41] = shiyongjun
