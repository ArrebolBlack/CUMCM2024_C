...
class data():
    ...
    def __init__(self):
        # 读取数据
        file_path_f1 = r'/home/ziwu/Newpython/C_2024/files/f_1.xlsx'
        farmland_data = pd.read_excel(file_path_f1, sheet_name='乡村的现有耕地')
        crop_data = pd.read_excel(file_path_f1, sheet_name='乡村种植的农作物')
        file_path_f2 = r'/home/ziwu/Newpython/C_2024/files/f_2.xlsx'
        file_path_nzw = r'/home/ziwu/Newpython/C_2024/files/农作物数据.xlsx'
        ...
        file_path_yield = r'/home/ziwu/Newpython/C_2024/files/mu_chan_liang.csv'
        file_path_price = r'/home/ziwu/Newpython/C_2024/files/danjia(aver).csv'
        file_path_cost = r'/home/ziwu/Newpython/C_2024/files/cheng_ben.csv'
        file_path_result = r'/home/ziwu/Newpython/C_2024/files/result_2023(1).csv'
        ...
        self.yield_matrix, self.price_matrix, self.cost_matrix, self.sale_matrix = self.calculate_future_matrixs()
    ...
    def calculate_future_matrixs(self):

        # 初始化未来的矩阵
        future_yield_matrix =  np.zeros((82, 41, 7))
        future_price_matrix =  np.zeros((82, 41, 7))
        future_cost_matrix  = np.zeros((82, 41, 7))
        future_sale_matrix = np.zeros((41, 7))

        # 预期销售量和亩产量波动
        for year in range(len(self.years)):
            # 农作物亩产量波动，较于2023（±10%）
            future_yield_matrix[:, :, year] = self.yield_matrix_2023 * np.random.uniform(0.9, 1.1)
                        # 1 + np.random.normal(0 ,0.005)) # +-10%

            # 小麦和玉米销售量增长5%-10%，其他作物±5%较2023
            for crop in range(self.result_2023.shape[1]):  # 针对每种作物
                if crop in [5, 6]:  # 需要替换成实际索引,玉米和小麦
                    growth_rate = np.random.uniform(0.05, 0.1)
                    if year == 0:
                        future_sale_matrix[ crop, year] = self.sale_2023[crop] * (1 + growth_rate)
                    else:
                        future_sale_matrix[ crop, year] = future_sale_matrix[crop, year - 1] * (1 + growth_rate )
                else:
                    future_sale_matrix[crop, year] = self.sale_2023[crop] * np.random.uniform(0.95, 1.05)
                                                     # (1 + np.random.normal(0 ,0.01)) # +-5%

            # 成本上涨5%
            future_cost_matrix[:, :, year] = self.cost_matrix_2023 * pow(1.05, year+1)

            # 销售价格：粮食类稳定，蔬菜类每年增长5%，食用菌价格下降1%-5%
            for crop in range(self.price_matrix_2023.shape[1]):
                if crop in range(0, 16):  # 粮食类索引
                    future_price_matrix[:, crop, year] = self.price_matrix_2023[:, crop]
                elif crop in range(16, 37):  # 蔬菜
                    future_price_matrix[:, crop, year] = (self.price_matrix_2023[:, crop] * pow(1.05, year+1))
                elif crop in range(37, 41):  # 食用菌
                    if crop == 40:
                        price_drop = 0.05
                    else:
                        price_drop = np.random.uniform(0.01, 0.05)
                    if year == 0:
                        future_price_matrix[:, crop, year] = self.price_matrix_2023[:, crop] * (1 - price_drop)
                    else:
                        future_price_matrix[:, crop, year] = future_price_matrix[:, crop, year - 1] * (1 - price_drop)

        return future_yield_matrix, future_price_matrix, future_cost_matrix, future_sale_matrix
