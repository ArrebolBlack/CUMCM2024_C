...
class EpisodesBuffer(Buffer):
    ...
    def push(self, **kwargs):
        ...
        buffer = self.buffer
        index = np.random.permutation(len(view))# 队友全观测
        # 随机排列，随机顺序处理
...
class MemoryGroup(object):
    ...
    def _flush(self, **kwargs):
        ...
        mask = np.where(kwargs['terminal'] == True, False, True)
        mask[-1] = False
    ...
    def push(self, **kwargs):
        for i, _id in enumerate(kwargs['ids']):
            ...
            if self.use_mean:
                self.agent[_id].append(obs0=kwargs['state'][0][i], feat0=kwargs['state'][1][i],
                                       act=kwargs['acts'][i], reward=kwargs['rewards'][i],
                                       alive=kwargs['alives'][i], prob=kwargs['prob'][i])
            else:
                self.agent[_id].append(obs0=kwargs['state'][0][i], feat0=kwargs['state'][1][i],
                                       act=kwargs['acts'][i], reward=kwargs['rewards'][i],
    ...
    def sample(self):
        ...
        feature = self.feat0.sample(next_idx)
...
class Runner(object):
    ...
    def run(self, variant_eps, iteration, win_cnt=None):
        ...
        if self.train:
            ...
            # if self.save_every and (iteraion + 1) % self.save_every == 0:
