...
def objective_function(action, yield_matrix, price_matrix, cost_matrix, sale_matrix, plot_areas):
    ...
    # real sale  1.2
    total_discount = 0.5 * np.sum(price_matrix_1 * (P1 - S1) + price_matrix_2 * (P2 - S2))

    # 计算总成本 亩数 x 成本 元/亩
    total_cost = np.sum(cost_matrix * real_planting_areas)
    # print(total_revenue, total_cost)

    # print("revenue:", total_revenue.shape)
    # print("cost:", total_cost.shape)
    # print("discount:", total_discount.shape)

    total_profit = total_revenue - total_cost + total_discount

    # print("total_profit:", total_profit.shape)
    ...
    # print("hsape;:", normalized_reward.shape)
    # Reward Clipping
    normalized_reward_clipped = reward_clipping(normalized_reward, -100, 2500)
