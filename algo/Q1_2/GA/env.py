...
import numpy as np
import pandas as pd
from Object_function_ import objective_function
...
class CropPlantingEnv(gym.Env):
    ...
    def __init__(self):
        ...
        # 使用 np.pad() 将其补零到 859
        padded_sale = np.pad(flattened_sale, (0, 859 - len(flattened_sale)), 'constant')
        self.obs_sale = padded_sale.reshape(859, 1)
        # 打印每个数组的形状以调试
        ...
        # Initialize
        # self.info_result_2023 = self.data.transition_3d(self.result_matrix_2023[:, :, np.newaxis])
